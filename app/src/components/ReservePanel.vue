<template>
  <section class="reserve-panel">
    <h2><van-icon :name="I.calendar" />预约景点清单</h2>
    <p class="empty">把想去的预约景点圈出来，系统会按当地游览顺序生成路线；选中预约景点时会自动加入对应城市。</p>
    <div class="reserve-toolbar">
      <span class="reserve-count"><van-icon :name="I.ticket" />已选 {{ state.reservationPicked.length }} 个预约景点</span>
      <van-button type="primary" size="small" :icon="I.route" @click="genRoute">生成预约路线</van-button>
    </div>
    ...
    <div ref="routeBox" class="route-result">
      <template v-if="state.showReserveRoute && reserveRouteGroups.length">
        <h3><van-icon :name="I.route" />我的预约游览路线</h3>
        <div v-for="g in reserveRouteGroups" :key="g.city.id" class="route-card">
          <div class="route-line"><van-icon :name="I.train" />{{ g.city.emoji }} {{ g.city.name }}：杭州出发 → {{ g.list.map(r => r.name).join(' → ') }} → 回杭州</div>
          <div class="empty">当地建议顺序：{{ g.list.map(r => r.route).join('；') }}</div>
          <div v-for="r in g.list" :key="r.id" class="route-step"><van-icon :name="I.location" /><strong>{{ r.name }}</strong><span>{{ r.window }} · {{ r.duration }} · {{ r.kind }}</span></div>
