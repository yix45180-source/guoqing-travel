<template>
  <section class="panel">
    ...
    <template v-if="state.planSlots.length">
      <div>
        <table>
          <thead><tr><th>预算粗算（{{ budget.days }} 天）</th><th>估算</th><th>口径</th></tr></thead>
          <tbody>
      ...
      <p v-if="leftoverNames.length" class="warn-line">这几座没排进当前天数：{{ leftoverNames.join('、') }}。可减少高峰留杭、增加天数，或拖掉某天换成它们。</p>
      <p v-else-if="!state.picked.some(id => id !== 'hz') && !state.picked.includes('hz')" class="warn-line">还没选外地城市：填满模式会用杭州主题日铺满；想去外地请先点城市卡片。</p>
    </template>
...
import { state, budget, leftoverNames, moveSlot } from '../store.js'
